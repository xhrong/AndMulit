...
import com.xhr.andmulti.encoder.AudioEncodeConfig;
import com.xhr.andmulti.encoder.AudioEncoder;
...
public class AudioRecorder implements Recorder {
    ...
    private Recorder.Callback mCallback;
    private CallbackDelegate mCallbackDelegate;
    private int mChannelsSampleRate;

    public AudioRecorder(AudioEncodeConfig config) {
...
//    }
    ...
    private static class CallbackDelegate extends Handler {
        ...
        CallbackDelegate(Looper l, Recorder.Callback callback) {
            super(l);
            this.mCallback = callback;
        }


        void onError(Recorder encoder, Exception exception) {
            Message.obtain(this, () -> {
                if (mCallback != null) {
        ...
        void onOutputFormatChanged(Recorder encoder, MediaFormat format) {
            Message.obtain(this, () -> {
                if (mCallback != null) {
        ...
        void onOutputBufferAvailable(Recorder encoder, int index, MediaCodec.BufferInfo info) {
            Message.obtain(this, () -> {
                if (mCallback != null) {
        ...
        private void offerOutput() {
            while (!mForceStop.get()) {
                ...
                if (index == INFO_OUTPUT_FORMAT_CHANGED) {
                    mCallbackDelegate.onOutputFormatChanged(AudioRecorder.this, mEncoder.getEncoder().getOutputFormat());
                }
                if (index < 0) {
                    info.set(0, 0, 0, 0);
                ...
                mCallbackDelegate.onOutputBufferAvailable(AudioRecorder.this, index, info);
